...
class CanvasOrderFormPage extends StatefulWidget {
  ...
  @override
  State<CanvasOrderFormPage> createState() => LayerOrderFormPage();
}

class LayerOrderFormPage extends State<CanvasOrderFormPage> {
        ...
        if (barang.fdQty != null && barang.fdQty! % 1 != 0) {
          if (barang.fdJenisSatuan == 2) {
            ...
            if (qtyGenapS % 1 != 0) {
              ...
              hargaAsli = await cbrg.getHargaAsliByFdKodeBarang(
              ...
              child: TextButton(
                onPressed: () async {
                  if (formInputKey.currentState!.validate()) {
                    ...
                    log('data: ${listKeranjang.map((item) => '(${item.fdKodeBarang}, ${item.fdNamaBarang}, ${item.fdHargaAsli}, ${item.fdUnitPrice}, ${item.fdTotalPrice}, ${item.fdUnitPriceK}, ${item.fdTotalPriceK}, promosi:${item.fdPromosi}, Qty: ${item.fdQty}, Satuan: ${item.fdJenisSatuan} ${item.fdNamaJenisSatuan} ${item.fdDiscount} ${item.fdDiscountDetail}, jnItem: ${item.jnItem})').toList()}');
                    if (isDecimal == 0) {
                      ...
                      for (var extra in fBarangExtra) {
                        ...
                        if (!exists) {
                          ...
                          hargaAsliExtra =
                              await cbrg.getHargaAsliByFdKodeBarang(
                                  extra.fdKodeBarang,
                                  0,
                                  widget.lgn.fdKodeLangganan!,
                                  extra.isHanger!);
