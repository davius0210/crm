...
import 'noo_ordersummaryform_page.dart';
import 'main.dart';
import 'noo_ordersummaryform_page.dart';
...
class NooOrderCartForm extends StatefulWidget {
  ...
  @override
  State<NooOrderCartForm> createState() => LayerOrderCartForm();
}

class LayerOrderCartForm extends State<NooOrderCartForm> {
  ...
  void initLoadPage() async {
    try {
      ...
      fdKodeHarga = await cbrg.getKodeHarga(widget.lgn.fdKodeNoo!);
      setState(() {
        isLoading = false;
      });
    } catch (e) {
                              ...
                              children: [
                                ...
                                Expanded(
                                    ...
                                    child: Padding(
                                      padding: const EdgeInsets.all(5),
                                      child: Text(
                                          'Rp. ${param.idNumberFormat.format(barang.fdUnitPrice).toString()}',
      ...
      bottomNavigationBar: isLoading
          ...
          : BottomAppBar(
              ...
              child: TextButton(
                onPressed: () async {
                  ...
                  totalDiscount =
                      JsonBarangDiskon.map((item) => item.fdDiscount)
                          .reduce((a, b) => a + b);
                  print(totalDiscount);
                  log('data: ${listKeranjang.map((item) => '(${item.fdKodeBarang}, ${item.fdNamaBarang}, Qty: ${item.fdQty}, Satuan: ${item.fdJenisSatuan})').toList()}');
                  if (checkSave) {
                    Navigator.push(
                            context,
                            MaterialPageRoute(
                                settings: const RouteSettings(
                                    name: 'NooOrderSummaryForm'),
                                builder: (context) => NooOrderSummaryForm(
                ...
                // Text('Summary Pesanan: ${widget.totalItem.toString()} items Rp. ${param.idNumberFormat.format(totalPesanan).toString()}'),
              ),
            ),
    );
  }
