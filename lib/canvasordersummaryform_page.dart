...
class CanvasOrderSummaryForm extends StatefulWidget {
  ...
  @override
  State<CanvasOrderSummaryForm> createState() => LayerOrderFormPage();
}

class LayerOrderFormPage extends State<CanvasOrderSummaryForm> {
  ...
  List<mbrg.BarangSelected> listKeranjangTotal = [];
  ...
  void initLoadPage() async {
    try {
      ...
      log('getdata: ${widget.listKeranjang.map((item) => '(${item.fdKodeBarang}, ${item.fdNamaBarang}, Qty: ${item.fdQty}, fdQtyStock: ${item.fdQtyStock}, fdQtyStockK: ${item.fdQtyStockK}, Satuan: ${item.fdJenisSatuan}, isHanger: ${item.isHanger})').toList()}');
      noFaktur =
      ...
      listAlamatKirim =
          await clgn.getAllLanggananAlamat(widget.lgn.fdKodeLangganan!);
      langganans =
  ...
  Future<void> saveLimitKredit() async {
    try {
      var dataLimitKredit = await clk
          .getDataLimitKreditByKodeLangganan(widget.lgn.fdKodeLangganan!);

      listOrder =
          await codr.getDataOrderByKodeLangganan(widget.lgn.fdKodeLangganan!);
      if (dataLimitKredit.isEmpty) {
        // overLK = widget.totalPesanan - sisaLK;
        gtpesananBaru = listOrder
            .first.fdTotalOrder; //sumpesananBaru + widget.totalPesanan ;
        overLK = gtpesananBaru - sisaLK;
        pengajuanBaru = limitKredit + overLK;
        sumpengajuanBaru = 0;
        sumpesananBaru = 0;
        sumoverLK = 0;
        gtpengajuanBaru = sumpengajuanBaru + pengajuanBaru;
        gtpesananBaru = sumpesananBaru + widget.totalPesanan;
        gtoverLK = sumoverLK + overLK;
      } else {
        pengajuanBaru = limitKredit + overLK;
        sumpengajuanBaru = (dataLimitKredit['fdPengajuanLimitBaru'] ?? 0);
        sumpesananBaru = (dataLimitKredit['fdPesananBaru'] ?? 0);
        sumoverLK = (dataLimitKredit['fdOverLK'] ?? 0);
      ...
      listLimitKredit = [
        mlk.LimitKredit(
          ...
          fdTanggal: DateFormat('yyyy-MM-dd')
    ...
    try {
      ...
      listOrder = [
        modr.Order(
          ...
          fdNoEntryOrder: widget.fdNoEntryOrder, //noEntry,
          ...
          //     0,
          //     (prev, item) =>
          //         prev + ((int.tryParse(item.fdQty ?? '0') as int))),
          fdQty: widget.listKeranjang.fold(
            0,
            (prev, item) =>
                prev +
          ...
          children: [
            ...
            Container(
              ...
              child: const Padding(
                  ...
                  child: Text(
                      'Cek transaksi sudah sesuai. Apakah Anda ingin membuat Invoice?')),
            ),
            ElevatedButton(
                onPressed: () async {
                  try {
                    ...
                    if (sisaLK < grandTotal) {
                      ...
                      //     MaterialPageRoute(
                      //         settings:
                      //             const RouteSettings(name: 'PiutangEdit'),
                      //         builder: (context) => PiutangEditPage(
                      //             user: widget.user,
                      //             lgn: widget.lgn,
                      //             noentry: noEntry.isNotEmpty
                      //                 ? noEntry
                ...
                onPressed: () async {
                  try {
                    ...
                    Navigator.push(
                        ...
                        MaterialPageRoute(
                            settings:
                                const RouteSettings(name: 'collectionedit'),
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      Padding(
                        padding: const EdgeInsets.all(5),
                        child: DropdownSearch<mlgn.LanggananAlamat>(
                          itemAsString: (mlgn.LanggananAlamat item) =>
                              item.fdAlamat!,
                          compareFn: (mlgn.LanggananAlamat item,
                                  mlgn.LanggananAlamat selectedItem) =>
                              item.fdAlamat == selectedItem.fdAlamat,
                          items: (String filter, LoadProps? loadProps) => listAlamatKirim,
                          selectedItem: listAlamatKirim.firstWhere(
                            (item) => item.fdAlamat!.trim() == alamatSelected,
                            orElse: () => mlgn.LanggananAlamat(
                              fdAlamat: '',
                              fdKodeLangganan: '',
                              fdJenis: '',
                              fdNoUrut: 0,
                          ...
                          onChanged: (mlgn.LanggananAlamat? value) {
                            setState(() {
                              alamatSelected = value?.fdAlamat!.trim();
                            });
                          },
                          filterFn: (mlgn.LanggananAlamat item, String filter) {
                            return item.fdAlamat!
                                    .toLowerCase()
                        ...
                        onPressed: () async {
                          ...
                          if (formInputKey.currentState!.validate()) {
                            grandTotal = widget.totalPesanan -
                                ...
                                orderExist;
                            print(
                                'widget.totalPesanan: ${widget.totalPesanan}');
                            print(
                                'widget.totalDiscount: ${widget.totalDiscount}');
                            print('orderExist: $orderExist');
                            print('sisaLK: $sisaLK');
                            print('Grand Total: $grandTotal');
                            if (sisaLK < grandTotal) {
                              _confirmLimitKredit(widget.lgn.fdKodeLangganan!);
                            } else {
                              await saveOrder();
                            }
                            if (!mounted) return;
