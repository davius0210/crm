...
import 'canvasordercartform_page.dart';
import 'canvasordersummaryform_page.dart';
import 'main.dart';
...
class CanvasOrderEditPage extends StatefulWidget {
  ...
  @override
  State<CanvasOrderEditPage> createState() => LayerOrderEditPage();
}

class LayerOrderEditPage extends State<CanvasOrderEditPage> {
  ...
  void initLoadPage() async {
    try {
      ...
      listBarang = await cbrg.getAllBarangStockCarry(
...
//cek listBarangSelected jika ada decimal
      ...
      log('getdata: ${listKeranjang.map((item) => '(${item.fdKodeBarang}, ${item.fdNamaBarang}, Qty: ${item.fdQty}, Price: ${item.fdUnitPrice}, Satuan: ${item.fdJenisSatuan}, isHanger: ${item.isHanger}, isShow: ${item.isShow}, jnItem: ${item.jnItem})').toList()}');
      log('listKeranjangJson: ${listKeranjangJson.map((item) => '(${item.fdKodeBarang}, ${item.fdNamaBarang}, Qty: ${item.fdQty}, Price: ${item.fdUnitPrice}, Satuan: ${item.fdJenisSatuan}, isHanger: ${item.isHanger}, isShow: ${item.isShow}, jnItem: ${item.jnItem})').toList()}');
  ...
  // Pastikan listBarang tidak null, jika null berikan list kosong []
  items: (String filter, LoadProps? loadProps) => listBarang,
  
  // Fungsi untuk menampilkan teks item saat dipilih
  itemAsString: (mbrg.BarangStock item) => item.fdNamaBarang,

  // Fungsi pembanding untuk menentukan item mana yang sedang terpilih
  compareFn: (mbrg.BarangStock item, mbrg.BarangStock selectedItem) =>
      item.fdKodeBarang.trim() == selectedItem.fdKodeBarang.trim(),

  // Perbaikan logika selectedItem agar tidak error jika tidak ditemukan
  selectedItem: listBarang.cast<mbrg.BarangStock?>().firstWhere(
    (item) => item?.fdKodeBarang.trim() == barangSelected,
    orElse: () => null, // Lebih baik return null jika tidak ditemukan
  ),

  // Konfigurasi Popup (Search Box, Item Builder)
  popupProps: PopupProps.menu(
    ...
    menuProps: MenuProps(shape: css.borderOutlineInputRound()),
  ),

  // Konfigurasi Tampilan Dropdown
  ...
  filterFn: (mbrg.BarangStock item, String filter) {
    ...
    return item.fdNamaBarang.toLowerCase().contains(filterLower) ||
           item.fdKodeBarang.toLowerCase().contains(filterLower);
  },
),
                      ),
                      const SizedBox(
                        ...
                        Navigator.push(
                            ...
                            MaterialPageRoute(
                                settings: const RouteSettings(
                                    name: 'CanvasOrderSummaryForm'),
                                builder: (context) => CanvasOrderSummaryForm(
                                    fdNoEntryOrder: widget.fdNoEntryOrder,
                                    lgn: widget.lgn,
                                    user: widget.user,
                                    ...
                                    navpage: '',
                                    routeName: 'CanvasOrderSummaryForm',
