...
// import 'package:crm_apps/stockverificationform_page.dart';
...
import 'stockverificationdetail_page.dart';
...
List<mstoc.Stock> _liststockverification = [];
...
class StockVerificationPage extends StatefulWidget {
  ...
  @override
  State<StockVerificationPage> createState() => Layerstockverification();
}

class Layerstockverification extends State<StockVerificationPage> {
  List<mstoc.Stock> _liststockverificationPending = [];
  List<mstoc.Stock> liststockverification = [];
  ...
  double gtoverLK = 0;
  double stockverificationExist = 0;
                  ...
                  try {
                    List<mstk.StockApi> stockApiList = [
                      mstk.StockApi(
                        ...
                        fdAlamatKirim:
                            _liststockverification[index].fdAlamatKirim,
                        fdKodeGudang:
                            _liststockverification[index].fdKodeGudang,
                        fdKodeGudangSF:
                            _liststockverification[index].fdKodeGudangSF,
                        fdNoUrutStock: 0,
                        fdKodeBarang: '',
                        fdNamaBarang: '',
                        fdPromosi: '',
                        ...
                        fdQtyStock: double.tryParse(
                                _liststockverification[index]
                                    ...
                                    .toString()) ??
                            0,
                        fdQtyStockK: _liststockverification[index].fdQtyStockK,
                        fdUnitPrice: 0,
                        fdUnitPriceK: 0,
                        fdBrutto: 0,
                        fdDiscount: 0,
                    ...
                    // mstk.StockApi result = await capi.sendstockverificationtoServer(
                    //     widget.user, '2', widget.startDayDate, stockApiList, 0);
  ...
  Future<void> initLoadPage() async {
    ...
    try {
      ...
      _liststockverification = await capi.getDataStockVerification(
          widget.user.fdToken,
          widget.user.fdKodeSF,
          'H',
          widget.user.fdKodeDepo,
          widget.startDayDate);
      if (_liststockverification.isEmpty) {
        if (!mounted) return;

        // ScaffoldMessenger.of(context).showSnackBar(
        //     const SnackBar(content: Text('Data Stock Verifikasi kosong')));
      } else {
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
        ...
        child: Scaffold(
            ...
            child: isLoading
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      ...
                      if (_liststockverificationPending.isNotEmpty) {
                        outletPending =
                            _liststockverificationPending.firstWhere(
                                (element) =>
                                        ...
                                        StockVerificationDetail(
                                            ...
                                            fdNoEntryStock:
                                                _liststockverification[index]
                                                    .fdNoOrder!,
                                            isEndDay: widget.isEndDay,
                                            routeName: 'stockverificationEdit',
                                            startDayDate: widget
                              ...
                              children: [
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      Text(
                                          'Stock Verification ID : ${_liststockverification[index].fdNoOrder}',
                                          style: css.textNormalBold(),
                                          softWrap: true),
                                      Text(
                                          'Tanggal Minta : ${_liststockverification[index].fdTanggalKirim != null ? param.dtFormatViewMMM.format(DateTime.parse(_liststockverification[index].fdTanggalKirim!)).toString() : 'Invalid Date'}',
                                          style: css.textNormalBold(),
                                          softWrap: true),
                                      Text(
                                          'Tanggal Kirim : ${_liststockverification[index].fdTanggalKirim != null ? param.dtFormatViewMMM.format(DateTime.parse(_liststockverification[index].fdTanggalKirim!)).toString() : 'Invalid Date'}',
                                          style: css.textNormalBold(),
                                          softWrap: true),
                                      Text(
                                          'Status : ${_liststockverification[index].fdKodeStatus == 0 ? 'Active' : 'Approve'}',
                                          style: css.textNormalBold(),
                                          softWrap: true),
                                    ],
                                  ),
          ...
          //                   MaterialPageRoute(
          //                       settings: const RouteSettings(
          //                           name: 'stockverificationForm'),
          //                       builder: (context) => StockVerificationDetail(
          //                           user: widget.user,
          //                           fdNoEntryStock: '',
          //                           isEndDay: widget.isEndDay,
          //                           routeName: 'stockverificationForm',
