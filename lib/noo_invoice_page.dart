...
class NooInvoicePage extends StatefulWidget {
  ...
  @override
  State<NooInvoicePage> createState() => LayerNooInvoicePage();
}

class LayerNooInvoicePage extends State<NooInvoicePage> {
  ...
  List<mbrg.BarangTOP> _listJtTop = [];
  ...
  void initLoadPage() async {
    try {
      ...
      totalCash = await ccoll.getCollectionByTipe(
          'Tunai', widget.lgn.fdKodeNoo!, widget.noentry);
      totalTransfer = await ccoll.getCollectionByTipe(
          'Transfer', widget.lgn.fdKodeNoo!, widget.noentry);
      totalGiro = await ccoll.getCollectionByTipe(
          'Giro', widget.lgn.fdKodeNoo!, widget.noentry);
      totalCheque = await ccoll.getCollectionByTipe(
          'Cheque', widget.lgn.fdKodeNoo!, widget.noentry);
      setState(() {
        isLoading = false;
      });
    } catch (e) {
  ...
  Future<void> getImageFromDevice() async {
    setState(() {
      selloutImg1 =
          '${param.appDir}/${param.imgPath}/${param.buktiTransferImgName(1, widget.user.fdKodeSF, widget.lgn.fdKodeNoo!, widget.noentry, widget.startDayDate)}';
      selloutImg2 =
          '${param.appDir}/${param.imgPath}/${param.buktiTransferImgName(2, widget.user.fdKodeSF, widget.lgn.fdKodeNoo!, widget.noentry, widget.startDayDate)}';
      selloutImg3 =
          '${param.appDir}/${param.imgPath}/${param.buktiTransferImgName(3, widget.user.fdKodeSF, widget.lgn.fdKodeNoo!, widget.noentry, widget.startDayDate)}';
    });
      ...
      onWillPop: !isLoading
          ? () async {
              Navigator.push(
                  context,
                  MaterialPageRoute(
                      settings: const RouteSettings(name: 'NooCanvas'),
                      builder: (context) => NooCanvasPage(
                          lgn: widget.lgn,
                          user: widget.user,
                      ...
                      Navigator.push(
                          ...
                          MaterialPageRoute(
                              settings: const RouteSettings(
                                  name: 'noocollectionallocation'),
                              builder: (context) => NooCollectionAllocationPage(
                                  lgn: widget.lgn,
                                  user: widget.user,
                                  routeName: 'noocollectionallocation',
                  ...
                  FloatingActionButton(
                    ...
                    onPressed: () {
                      Navigator.push(
                          ...
                          MaterialPageRoute(
                              settings: const RouteSettings(
                                  name: 'NooCollectionPage'),
                              builder: (context) => NooCollectionPage(
                                  lgn: widget.lgn,
                                  user: widget.user,
                                  routeName: 'NooCollectionPage',
                                  navpage: widget.navpage,
                                  noentry: widget.noentry,
                                  startDayDate: widget.startDayDate,
                                  endTimeVisit: widget.endTimeVisit,
