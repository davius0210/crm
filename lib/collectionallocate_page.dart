...
class CollectionAllocationPage extends StatefulWidget {
  ...
  const CollectionAllocationPage(
      {super.key,
      required this.lgn,
      required this.user,
...
class LayerCollectionAllocation extends State<CollectionAllocationPage> {
  bool isLoading = false;
  bool autoAllocate = false;
  double allocationUsed = 0.0;
  double totalPiutang = 0;
  List<mpiu.Payment> listPayment = [];
  List<mpiu.Payment> listPaymentDetail = [];
  ...
  void initLoadPage() async {
    try {
      ...
      if (widget.navpage == 'piutangedit') {
        listPaymentDetail = await cpiu.getAllPaymentDetail(
            widget.lgn.fdKodeLangganan!, widget.noentry, widget.navpage);
      } else {
        // listPaymentDetail = await cpiu.getAllPaymentDetail(
        //     widget.lgn.fdKodeLangganan!, 'noocanvas');
        listPaymentDetail = await cpiu.getAllPaymentDetail(
            widget.lgn.fdKodeLangganan!, widget.noentry, 'noocanvas');
      }
      ...
      onWillPop: () async {
        if (widget.navpage == 'piutangedit') {
          Navigator.push(
              context,
              MaterialPageRoute(
                  settings: const RouteSettings(name: 'PiutangEdit'),
                  builder: (context) => PiutangEditPage(
                      ...
                      routeName: 'PiutangEdit'))).then((value) {
            initLoadPage();
