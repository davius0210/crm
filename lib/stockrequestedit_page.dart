...
import 'controller/Stock_cont.dart' as codr;
...
import 'models/stock.dart' as modr;
...
class StockEditPage extends StatefulWidget {
  ...
  @override
  State<StockEditPage> createState() => LayerStockEditPage();
}

class LayerStockEditPage extends State<StockEditPage> {
  bool isLoading = false;
  List<mlgn.Langganan> langganans = [];
  List<modr.Stock> listStock = [];
  ...
  TextEditingController txtSisaStock = TextEditingController();
  TextEditingController txtPesananBaru = TextEditingController();
  TextEditingController txtOverStock = TextEditingController();
  ...
  void initLoadPage() async {
    try {
      ...
      listBarangSelected =
          await cbrg.getAllBarangInputOrder(widget.fdNoEntryStock);
      satuanPerBarang =
...
//cek listBarangSelected jika ada decimal
      ...
      tempBarangSelected = listBarangSelected.map((barang) {
        return mbrg.BarangSelected(
          ...
          fdTotalPriceK: barang.fdQtyK! * barang.fdUnitPriceK!.toDouble(), // 0,
          ...
          if (barang.fdQty != null && barang.fdQty! % 1 != 0) {
            ...
            if (barang.fdJenisSatuan == 2) {
              ...
              if (qtyGenapS % 1 != 0) {
                ...
                // barang.fdNamaJenisSatuan = satuanMatch.fdNamaSatuan;
                // barang.fdJenisSatuan =
                //     int.tryParse(satuanStockS.toString()) ?? barang.fdJenisSatuan;
              ...
              if (qtyGenapK % 1 != 0) {
                ...
                var satuan = await cbrg.getSatuanBarangByFdKodeBarangMap(
                    barang.fdKodeBarang.trim());
                // Cari fdNamaSatuan dari satuan berdasarkan satuanStock
                // final satuanStockKode = satuanStockK.toString();
                // final satuanList = satuan[barang.fdKodeBarang.trim()] ?? [];
                // final satuanMatch = satuanList.firstWhere(
                //   (s) => s.fdKodeSatuan.toString() == satuanStockKode,
                //   orElse: () => mbrg.Satuan(fdKodeSatuan: '', fdNamaSatuan: ''),
                // );
                // barang.fdJenisSatuan = int.tryParse(satuanStockK);
                // barang.fdNamaJenisSatuan = satuanMatch.fdNamaSatuan;
                // barang.fdJenisSatuan =
                //     int.tryParse(satuanStockK.toString()) ?? barang.fdJenisSatuan;
      ...
      log('getdata: ${listKeranjang.map((item) => '(${item.fdKodeBarang}, ${item.fdNamaBarang}, Qty: ${item.fdQty}, Price: ${item.fdUnitPrice}, Satuan: ${item.fdJenisSatuan}, isHanger: ${item.isHanger}, isShow: ${item.isShow})').toList()}');
      log('listKeranjangJson: ${listKeranjangJson.map((item) => '(${item.fdKodeBarang}, ${item.fdNamaBarang}, Qty: ${item.fdQty}, Price: ${item.fdUnitPrice}, Satuan: ${item.fdJenisSatuan}, isHanger: ${item.isHanger}, isShow: ${item.isShow})').toList()}');
...
// [{"fdKodeDepo":"01","fdKodeSF":"516020","fdKodeBarang":"LDL7AB1402"},{"fdKodeDepo":"01","fdKodeSF":"516020","fdKodeBarang":"LDL4AB1403"}]
                      ...
                      //update fdNamaJenisSatuan sesuai form Stock awal
...
//hitung total pesanan
                      ...
                      for (int index = 0;
                        ...
                        print('totalStockRequest: $totalStockRequest');
                        totalDiscount += 0;
                      ...
                      if (checkSave) {
                        ...
                        //     MaterialPageRoute(
                        //         settings:
                        //             const RouteSettings(name: 'StockCartForm'),
                        //         builder: (context) => StockCartForm(
                        //             fdNoEntryStock: widget.fdNoEntryStock,
                        //             lgn: widget.lgn,
                        //             user: widget.user,
                        //             totalItem: totalItem,
